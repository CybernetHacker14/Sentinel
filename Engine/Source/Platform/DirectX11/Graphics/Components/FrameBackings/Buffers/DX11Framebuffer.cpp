...
namespace Sentinel {
    ...
    namespace Utils {
        static void AttachColorTexture(
            ...
            DX11Common::GetDevice()->CreateRenderTargetView(
                outColorAttachment->RenderTexture.Get(), &rTVDescription, &outColorAttachment->RenderTargetView);
            ...
            // SRV
            {
                ...
                DX11Common::GetDevice()->CreateShaderResourceView(
                    outColorAttachment->RenderTexture.Get(), &sRVDescription, &outColorAttachment->ShaderResourceView);
            }
        }
        ...
        static void AttachDepthTexture(
            ...
            DX11Common::GetDevice()->CreateDepthStencilView(
                outDepthAttachment->DepthStencilBuffer.Get(),
                &depthStencilDescription,
                &outDepthAttachment->DepthStencilView);
        }
        ...
        static Bool IsColorFormat(TextureFormat format) {
            return (
                format == TextureFormat::RGBA32F || format == TextureFormat::RGBA8UNORM ||
                format == TextureFormat::R32SINT);
        }

        static Bool IsDepthFormat(TextureFormat format) { return (format == TextureFormat::D24S8UINT); }
    }  // namespace Utils

    DX11Framebuffer::DX11Framebuffer(const FramebufferSpecification& specification) : Framebuffer(specification) {
        ...
        m_GetDepthAttachmentFunction = ST_BIND_EVENT_FN(GetDepthAttachment);
            ...
            for (UInt32 i = 0; i < m_ColorAttachments.size(); i++) {
                ...
                if (Utils::IsColorFormat(m_ColorAttachmentSpecifications[i].TextureFormat))
                    Utils::AttachColorTexture(
                        m_ColorAttachmentSpecifications[i], m_Specification, &m_ColorAttachments[i]);
            }
        }
    ...
    void DX11Framebuffer::Bind() const {
        ...
        for (UInt32 i = 0; i < m_ColorAttachments.size(); i++) pRenderViews[i] = m_ColorAttachments[i].RenderTargetView;

        DX11Common::GetContext()->OMSetRenderTargets(
            static_cast<UInt32>(m_ColorAttachments.size()),
            pRenderViews[0].GetAddressOf(),
            m_DepthAttachment.DepthStencilView.Get());
    }
    ...
    void DX11Framebuffer::BindColorBuffer(UInt32 index, UInt32 slot, ShaderType shaderType) {
        switch (shaderType) {
            case ShaderType::VERTEX:
                DX11Common::GetContext()->VSSetShaderResources(
                    slot, 1, m_ColorAttachments[index].ShaderResourceView.GetAddressOf());
                break;

            case ShaderType::PIXEL:
                DX11Common::GetContext()->PSSetShaderResources(
                    slot, 1, m_ColorAttachments[index].ShaderResourceView.GetAddressOf());
                break;

            case ShaderType::COMPUTE:
                DX11Common::GetContext()->CSSetShaderResources(
                    slot, 1, m_ColorAttachments[index].ShaderResourceView.GetAddressOf());
                break;
    ...
    void DX11Framebuffer::BindDepthBuffer(UInt32 slot, ShaderType shaderType) {
        switch (shaderType) {
            case ShaderType::VERTEX:
                DX11Common::GetContext()->VSSetShaderResources(
                    slot, 1, m_DepthAttachment.ShaderResourceView.GetAddressOf());
                break;
    ...
    void DX11Framebuffer::UnbindBuffer(UInt32 slot, ShaderType shaderType) {
        switch (shaderType) {
            case ShaderType::VERTEX: DX11Common::GetContext()->VSSetShaderResources(slot, 1, nullptr); break;

            case ShaderType::PIXEL: DX11Common::GetContext()->PSSetShaderResources(slot, 1, nullptr); break;

            case ShaderType::COMPUTE: DX11Common::GetContext()->CSSetShaderResources(slot, 1, nullptr); break;

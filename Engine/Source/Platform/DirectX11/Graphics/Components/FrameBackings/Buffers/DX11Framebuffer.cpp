...
namespace Sentinel
{
		...
		if (!m_ColorAttachmentSpecifications.empty())
		{
			...
			for (UInt32 i = 0; i < m_ColorAttachments.size(); i++)
			{
				...
				if (Utils::IsColorFormat(m_ColorAttachmentSpecifications[i].TextureFormat))
					Utils::AttachColorTexture(m_ColorAttachmentSpecifications[i],
						m_Specification, &m_ColorAttachments[i]);
			}
		}
	...
	void DX11Framebuffer::BindDepthBuffer(UInt32 slot, ShaderType shaderType) {
		...
		{
			case ShaderType::VERTEX:
				DX11Common::GetContext()->VSSetShaderResources(slot, 1,
					m_DepthAttachment.ShaderResourceView.GetAddressOf());
				break;

			case ShaderType::PIXEL:
				DX11Common::GetContext()->PSSetShaderResources(slot, 1,
					m_DepthAttachment.ShaderResourceView.GetAddressOf());
				break;

			case ShaderType::COMPUTE:
				DX11Common::GetContext()->CSSetShaderResources(slot, 1,
					m_DepthAttachment.ShaderResourceView.GetAddressOf());
				break;

...
namespace Sentinel {
    namespace BitUtils {
        ...
        static UInt8 GetFirstTrailingSetBit(UInt32 bits) {
            return MultiplyDeBruijnBitPosition[((UInt32)((bits & (0 - bits)) * 0x077CB531U)) >> 27];
        }

        static UInt8 GetFirstTrailingNonSetBit(UInt32 bits) {
            return MultiplyDeBruijnBitPosition[((UInt32)(((~bits) & (0 - (~bits))) * 0x077CB531U)) >> 27];
        }

        static UInt8 CountSetBits(UInt32 bits) {
#ifdef ST_COMPILER_MSVC
    ...
    public:
        inline void Allocate(UInt8 count) {
        ...
        template<typename... Args>
        inline T* New(UInt8& outIndex, Args&&... args) {
            ST_BREAKPOINT_ASSERT(m_Occupied <= m_Max, "Max count reached")
        ...
        template<typename... Args>
        inline void Delete(UInt8 index, Args&&... args) {
        ...
        inline T* operator[](const UInt8& index) const {
            T* start = (T*)m_FirstAddress;
            return &(start[index]);
        }

        inline const UInt8 GetAllocations() const { return m_Occupied; }

        inline const UInt8 GetFreeCount() const { return m_Free; }

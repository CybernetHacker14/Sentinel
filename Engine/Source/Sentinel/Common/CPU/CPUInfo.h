...
namespace Sentinel
{
	...
	public:
		...
		inline static void CalculateFeatures() {
		...
		inline static const char* GetCPUType() {
			static char type[13];
			...
			if (!type[0])
			{
				...
				if (CPUIDMaxFunction > 0)
				{
					...
					(void)a;
					type[i++] = (char)(b & 0xff); b >>= 8;
					type[i++] = (char)(b & 0xff); b >>= 8;
					type[i++] = (char)(b & 0xff); b >>= 8;
					type[i++] = (char)(b & 0xff);

					type[i++] = (char)(d & 0xff); d >>= 8;
					type[i++] = (char)(d & 0xff); d >>= 8;
					type[i++] = (char)(d & 0xff); d >>= 8;
					type[i++] = (char)(d & 0xff);

					type[i++] = (char)(c & 0xff); c >>= 8;
					type[i++] = (char)(c & 0xff); c >>= 8;
					type[i++] = (char)(c & 0xff); c >>= 8;
					type[i++] = (char)(c & 0xff);
				}
				...
				if (!type[0])
				{
					strcpy_s(type, sizeof(type), "Unknown");
				}
			}
		...
		inline static UInt32 GetCacheLineSize() {

	...
	namespace STL
	{
		...
		public:
			...
			template <class C, typename =
				typename enable_if < is_class<C>{} > ::type >
			...
			template <
				...
				!is_same<delegate, typename decay<T>::type>{}
				> ::type
			>
					delegate(T&& f) :
		...
		private:
			...
			template <class C>
			struct is_member_pair< pair<C* const,
				R(C::* const)(A...)> > : true_type {};

			template <typename>
			struct is_const_member_pair : false_type {};

			template <class C>
			struct is_const_member_pair< pair<C const* const,
				R(C::* const)(A...) const> > : true_type {};
...
namespace std
{
	...
	struct hash<Sentinel::STL::delegate<R(A...)> > {
		size_t operator()(Sentinel::STL::delegate<R(A...)> const& d) const noexcept {
			...
			return hash<typename Sentinel::STL::delegate<R(A...)>::stub_ptr_type>()(

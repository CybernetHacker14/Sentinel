...
namespace Sentinel {
    ...
    Bool VirtualFilesystem::ResolvePhysicalPath(const STL::string& virtualPath, STL::string& outPhysicalPath) {
        ...
        const STL::string& remainderString =
            virtualPath.substr(virtualDirectory.size() + 2, virtualPath.size() - virtualDirectory.size());
    ...
    Bool VirtualFilesystem::WriteToFile(const STL::string& path, UInt8* buffer) {
        STL::string physicalPath;
        return false;
        // return ResolvePhysicalPath(path, physicalPath) ? Filesystem::WriteToFileAtPath(path.c_str(), buffer,
        // sizeof(buffer)) : false;
    }

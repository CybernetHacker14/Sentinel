...
// Approach 1
// 1. Segregate base systems like rendering(visuals, text, UI), physics, animation, audio, AI, logging, etc. in one
// static lib
// 2. Segregate tool-set systems like Editor UI(viewports, panels), gizmos, in another module.
// 3. Work some sort of logic to embed output of editor(data and logic) in a seperate executable which will use the
// engine module.
...
// 3. During final build, the toolset part of the editor will be stripped away (using I guess - defines), the
// viewport settings will be changed, and we get the final game.

// Things to note from Discord
// @Nekko - i would probably generally have 2 separate binaries, 1 for editor, 1 without editor. Game logic would be
// loaded at runtime both of the binaries would come from 1 source tho, with editor specific stuff enclosed in an
// #ifdef block
...
int main(int argc, char** argv) {
    ...
    // TODO : Pass the cmd value arguments
    // TODO : Change this to WinMain (just for fun though)
